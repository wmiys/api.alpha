...
def getInternal(product_request_id: UUID) -> models.ProductRequestInternal | None:
    ...
    product_request_model = getModel(db_result.data, DbColumnPrefix.PRODUCT_REQUEST, serializers.ProductRequestSerializer)
    ...
    pr_internal.renter  = getModel(db_result.data, DbColumnPrefix.RENTER, serializers.UserSerializer)
    pr_internal.lender  = getModel(db_result.data, DbColumnPrefix.LENDER, serializers.UserSerializer)
    pr_internal.payment = getModel(db_result.data, DbColumnPrefix.PAYMENT, serializers.PaymentSerializer)
...
#------------------------------------------------------
def getModel(data: dict, prefix: DbColumnPrefix, SerializerClass: serializers.SerializerBase):
    # get the subset of key/values that have the specified prefix
    namespace_dict = get_namespace(data, prefix.value)
...
#-----------------------------------------------------
def get_namespace(data: dict, namespace: str, lowercase: bool=True, trim: bool=True) -> dict:
    return_value = {}
